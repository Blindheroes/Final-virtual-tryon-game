...
icon = cv2.imread("Baju 2D/under-man/jeans-full.png", cv2.IMREAD_UNCHANGED)
if icon is None:
    raise FileNotFoundError("Icon not found")

shirt = cv2.imread("Baju 2D/under-man/layered-denim-midpart 1.png", cv2.IMREAD_UNCHANGED)
if shirt is None:
    raise FileNotFoundError("Shirt image not found. Check the path.")

pants = cv2.imread("Baju 2D/under-man/flaredjeans.png", cv2.IMREAD_UNCHANGED)
if pants is None:
    raise FileNotFoundError("Pants image not found. Check the path.")

right_sleeve = cv2.imread("Baju 2D/under-man/arm-denim 1.png", cv2.IMREAD_UNCHANGED)
if right_sleeve is None:
    raise FileNotFoundError("right_sleeve image not found.")

left_sleeve = cv2.imread("Baju 2D/under-man/arm-denim 2.png", cv2.IMREAD_UNCHANGED)
...
while True:
    ...
    caption = ("Pria dengan tubuh kurus dianjurkan memilih pakaian berwarna cerah untuk menciptakan ilusi tubuh yang lebih berisi. Penggunaan teknik layering, seperti mengenakan jaket atau blazer di atas kaos, juga dapat memberikan volume tambahan pada penampilan. Pilihan pakaian sebaiknya berpotongan slim fit, namun tidak terlalu ketat atau longgar. Motif horizontal atau geometris lebih disarankan daripada motif garis vertikal, karena dapat mengurangi kesan tubuh yang terlalu ramping. Selain itu, bahan pakaian yang agak tebal, seperti denim, juga dapat membantu menambah dimensi pada tubuh.")
