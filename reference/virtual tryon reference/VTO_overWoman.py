...
icon = cv2.imread("Baju 2D/over-woman/full-cloth.png", cv2.IMREAD_UNCHANGED)
if icon is None:
    raise FileNotFoundError("Icon not found")


shirt = cv2.imread("Baju 2D/over-woman/dark-shit-midpart.png", cv2.IMREAD_UNCHANGED)
if shirt is None:
    raise FileNotFoundError("Shirt image not found. Check the path.")

pants = cv2.imread("Baju 2D/over-woman/highwaist 1.png", cv2.IMREAD_UNCHANGED)
if pants is None:
    raise FileNotFoundError("Pants image not found. Check the path.")

right_sleeve = cv2.imread("Baju 2D/over-woman/dark-shirt-arm.png", cv2.IMREAD_UNCHANGED)
if right_sleeve is None:
    raise FileNotFoundError("right_sleeve image not found.")

left_sleeve = cv2.imread("Baju 2D/over-woman/dark-shirt-arm-1.png", cv2.IMREAD_UNCHANGED)
...
while True:
    ...
    if results.pose_landmarks:
        ...
        left_hip = np.array([
            ...
            lm[mp_pose.PoseLandmark.LEFT_HIP.value].y * h + 30
        ], dtype=np.float32)
        right_hip = np.array([
            lm[mp_pose.PoseLandmark.RIGHT_HIP.value].x * w,
            lm[mp_pose.PoseLandmark.RIGHT_HIP.value].y * h + 30
        ], dtype=np.float32)
    ...
    caption = ("Wanita dengan tubuh berisi disarankan memilih atasan oversize yang tidak terlalu menonjolkan lekuk tubuh. Celana high waist dapat memberikan efek pinggang yang lebih ramping. Warna-warna netral atau gelap pada pakaian juga dapat membantu menciptakan kesan lebih ramping. Pilihlah bahan pakaian yang jatuh dan hindari bahan tebal atau motif besar.")
